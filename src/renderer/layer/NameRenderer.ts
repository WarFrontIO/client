	...
	render(context: GameGLContext): void {
		...
		for (let i = 0; i < playerNameRenderingManager.playerData.length; i++) {
			...
			if (player && player.isAlive()) {
				...
				toDraw.push({string: player.name, nameX: data.nameX, nameY: data.nameY, size: data.size, baselineBottom: true});
				toDraw.push({string: formatTroops(player.getTroops()), nameX: data.nameX, nameY: data.nameY + data.size / 2, size: data.size});
			}
		}
